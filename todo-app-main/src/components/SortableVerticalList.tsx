...
export const SortableVerticalList = <T extends Record<string, any>>(props: SortableListProps<T>) => {
  const mergedProps = mergeProps({ idLabel: "id" }, props)
  ...
  return (
    ...
    >
      ...
      <DragOverlay class={mergedProps.overlayClass}>
        <div class="sortable">{activeItem() ? mergedProps.itemComponent(activeItem()!) : null}</div>
      </DragOverlay>
    </DragDropProvider>
  );
};